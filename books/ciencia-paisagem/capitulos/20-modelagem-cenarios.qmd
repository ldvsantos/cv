...
%%| fig-width: 9
flowchart TD
    A["Cenários Paisagísticos"] --> B["Tendencial<br>(business as usual)<br>O que acontece se<br>nada mudar?"]
    A --> C["Otimista<br>(conservação ativa)<br>O que acontece se<br>implementarmos<br>políticas de conservação?"]
    A --> D["Pessimista<br>(desregulação)<br>O que acontece se<br>a fiscalização<br>falhar?"]

    B --> E["Projeção baseada<br>em taxas históricas"]
    C --> F["Simulação com<br>restrições ambientais<br>e restauração"]
    D --> G["Simulação com<br>remoção de barreiras<br>ao desmatamento"]

    style A fill:#2E7D32,color:#fff
```
Três tipos de cenários paisagísticos utilizados em planejamento territorial, variando de tendencial (projeção do status quo) a contrastantes (otimista e pessimista).
:::
