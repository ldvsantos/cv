...
```{dot}
//| label: fig-resiliencia-paisagem
//| fig-cap: "Comparação entre uma paisagem frágil (homogênea, fragmentada) e uma paisagem resiliente (diversificada, conectada) em resposta a perturbações climáticas."
//| fig-width: 6.5
digraph {
    rankdir=TB
    bgcolor="transparent"
    ...
    subgraph cluster_fragil {
        label="Paisagem Frágil"
        fontname="Helvetica"
        fontsize=12
        style="rounded,dashed"
        color="#C62828"

        A1 [label="Monocultura\ndominante", fillcolor="#FFCDD2", color="#C62828"]
        B1 [label="Baixa diversidade\nde cobertura", fillcolor="#FFCDD2", color="#C62828"]
        C1 [label="Fragmentos isolados\nsem conectividade", fillcolor="#FFCDD2", color="#C62828"]
        D1 [label="Alta vulnerabilidade\na extremos climáticos", fillcolor="#B71C1C", fontcolor="white", color="#7F0000", penwidth=1.5]

        A1 -> B1 -> C1 -> D1
    }

    subgraph cluster_resiliente {
        label="Paisagem Resiliente"
        fontname="Helvetica"
        fontsize=12
        style="rounded,dashed"
        color="#2E7D32"

        A2 [label="Mosaico\ndiversificado", fillcolor="#C8E6C9", color="#2E7D32"]
        B2 [label="Alta diversidade\nde cobertura", fillcolor="#C8E6C9", color="#2E7D32"]
        C2 [label="Fragmentos conectados\npor corredores", fillcolor="#C8E6C9", color="#2E7D32"]
        D2 [label="Capacidade de\nabsorção e adaptação", fillcolor="#2E7D32", fontcolor="white", color="#1B5E20", penwidth=1.5]

        A2 -> B2 -> C2 -> D2
    }

    E [label="Perturbação climática\n(seca, calor, tempestade)", fillcolor="#FFF9C4", color="#F57F17"]
    F1 [label="Colapso funcional\nperda de produção\ne serviços", fillcolor="#B71C1C", fontcolor="white", color="#7F0000"]
    F2 [label="Recuperação\nmanutenção de\nfunções essenciais", fillcolor="#2E7D32", fontcolor="white", color="#1B5E20"]
