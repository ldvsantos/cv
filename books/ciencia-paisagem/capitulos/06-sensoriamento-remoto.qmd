...
```{dot}
//| label: fig-assinatura-espectral
//| fig-cap: "Curvas de reflectância espectral típicas de vegetação fotossinteticamente ativa, solo exposto e água limpa. Valores ilustrativos para demonstrar os contrastes espectrais que permitem a discriminação de coberturas."
//| fig-width: 6.5
digraph {
    bgcolor="transparent"
    ...
    label="Assinaturas Espectrais Típicas\nReflectância (%) por banda espectral"
    labelloc=t
    fontname="Helvetica"
    fontsize=13
    ...
    tbl [label=<
        <TABLE BORDER="1" CELLBORDER="1" CELLSPACING="0" CELLPADDING="8" BGCOLOR="white">
            <TR>
                <TD BGCOLOR="#EEEEEE"><B>Cobertura</B></TD>
                <TD BGCOLOR="#3F51B5" WIDTH="55"><FONT COLOR="white"><B>Azul<BR/>0.45 µm</B></FONT></TD>
                <TD BGCOLOR="#4CAF50" WIDTH="55"><FONT COLOR="white"><B>Verde<BR/>0.55 µm</B></FONT></TD>
                <TD BGCOLOR="#F44336" WIDTH="55"><FONT COLOR="white"><B>Verm.<BR/>0.65 µm</B></FONT></TD>
                <TD BGCOLOR="#795548" WIDTH="55"><FONT COLOR="white"><B>NIR<BR/>0.85 µm</B></FONT></TD>
                <TD BGCOLOR="#607D8B" WIDTH="55"><FONT COLOR="white"><B>SWIR1<BR/>1.6 µm</B></FONT></TD>
                <TD BGCOLOR="#455A64" WIDTH="55"><FONT COLOR="white"><B>SWIR2<BR/>2.2 µm</B></FONT></TD>
            </TR>
            <TR>
                <TD BGCOLOR="#E8F5E9"><B>Vegetação</B></TD>
                <TD>3%</TD><TD>8%</TD><TD>4%</TD><TD BGCOLOR="#C8E6C9"><B>50%</B></TD><TD>25%</TD><TD>10%</TD>
            </TR>
            <TR>
                <TD BGCOLOR="#FFF3E0"><B>Solo exposto</B></TD>
                <TD>10%</TD><TD>15%</TD><TD>20%</TD><TD>25%</TD><TD BGCOLOR="#FFE0B2"><B>30%</B></TD><TD>28%</TD>
            </TR>
            <TR>
                <TD BGCOLOR="#E3F2FD"><B>Água</B></TD>
                <TD>5%</TD><TD>4%</TD><TD>3%</TD><TD>1%</TD><TD>0%</TD><TD>0%</TD>
            </TR>
        </TABLE>
    >]
}
...
Índices espectrais são combinações algébricas de bandas de reflectância que realçam características específicas da superfície e reduzem a dimensionalidade da matriz de dados original. O vetor analítico autônomo mais empregado em escalas de paisagem é o NDVI (*Normalized Difference Vegetation Index*), projetado para explorar o espalhamento fotônico gerado pela estrutura mesofilar saudável contrastada com a taxa de absorção de clorofila ativa.

$$
NDVI = \frac{\rho_{NIR} - \rho_{RED}}{\rho_{NIR} + \rho_{RED}}
$$

Mecanisticamente, $\rho_{NIR}$ e $\rho_{RED}$ referem-se às frações de reflectância de superfície (pós-correção atmosférica) no domínio do infravermelho próximo e do vermelho. 
**Exemplo de cálculo:** Imagine um pixel sobre uma roça agroflorestal em máxima produtividade foliar, resultando em intensa absorção fotossintética no vermelho ($\rho_{RED} = 0.05$) e massiva reflexão infravermelha pelo mesofilo esponjoso sem danos hídricos ($\rho_{NIR} = 0.45$). A aplicação resulta em $NDVI = (0.45 - 0.05) / (0.45 + 0.05) = 0.40 / 0.50 = 0.80$. Inversamente, se o mesmo pixel sofrer colapso estrutural por seca ou colheita e expuser solo, ambas as reflectâncias se equalizam próximo a 0.20, retraindo o NDVI a zero ou para taxas negativas caso se torne alagado. A normalização no denominador mitiga o ruído anisotrópico causado por variações angulares de iluminação, topografia oblíqua e albedo do substrato basal. Contudo, em dosséis hiperdensos onde o Índice de Área Foliar (IAF) excede 3, o limite de absorção do pigmento colapsa assintoticamente, bloqueando qualquer variação no algoritmo. Para romper essa barreira, o EVI (*Enhanced Vegetation Index*) reconfigurou a sensibilidade através de parâmetros restritivos.

$$
EVI = G \cdot \frac{\rho_{NIR} - \rho_{RED}}{\rho_{NIR} + C_1 \cdot \rho_{RED} - C_2 \cdot \rho_{BLUE} + L}
$$

A equação introduz regressores analíticos para contornar ruídos que o NDVI absorve passivamente: $G = 2{,}5$ amplifica o ganho direcional, os coeficientes $C_1 = 6$ e $C_2 = 7{,}5$ extraem a interferência de aerossóis empregando a banda azul térmica ($\rho_{BLUE}$) como referência calibradora, e $L = 1$ intercepta e diminui a assinatura isolada do solo quando a biomassa é delgada ou o plantio apresenta falhas. Diferente do cômputo fechado da proporção anterior, o EVI exige calibração rigorosa de quatro bandas de reflectância de superfície (BOA), sendo essencial para modelar gradientes em matrizes florestais clímax, visto que não sofre achatamento assintótico ante altos valores de cobertura radicular da Amazônia ou da Mata Atlântica.

Séries temporais desses descritores orbitais monitoram passivamente o comportamento fenológico da vizinhança ecossistêmica. No Cerrado e na Caatinga, a vegetação decídua apresenta queda acentuada de NDVI na estação seca (amplitude sazonal de 0,3–0,5), enquanto formações perenifólias (matas de galeria, florestas ombrófilas) mantêm NDVI relativamente estável ao longo do ano (amplitude inferior a 0,1). Essa informação fenológica é invaluável para a classificação de fitofisionomias e para a distinção entre pastagem (ciclo fenológico governado pelo manejo) e vegetação nativa (ciclo governado pela chuva). A composição em falsa cor (Fig. @fig-sentinel2-falsa-cor) ilustra como a combinação de bandas espectrais do Sentinel-2 realça feições da paisagem que não são visíveis na composição em cor verdadeira, permitindo discriminar vegetação sadia (tons vermelhos), solo exposto (tons claros) e corpos d'água (tons escuros).

![Composição em falsa cor de imagem Sentinel-2: a combinação de bandas do infravermelho próximo com o vermelho e o verde realça a vegetação fotossinteticamente ativa (tons vermelhos), permitindo discriminar fitofisionomias, solo exposto e corpos d'água com maior contraste do que a composição em cor verdadeira.](../img/sentinel2_composicao_falsa_cor.png){#fig-sentinel2-falsa-cor fig-align="center" width="85%"}
