...
::: {#fig-abordagens-mudanca}
```{mermaid}
%%| fig-width: 8
flowchart TD
    A["Detecção de Mudanças"] --> B["Pré-classificação<br>(Comparação de mapas)"]
    A --> C["Pós-classificação<br>(Diferença de imagens)"]
    A --> D["Séries temporais<br>(Análise de trajetória)"]
    B --> E["Transição A→B<br>entre datas"]
    C --> F["Diferença de NDVI,<br>razão de bandas"]
    D --> G["BFAST, LandTrendr,<br>CCDC"]

    style A fill:#2E7D32,color:#fff
```
Três famílias de abordagens para detecção de mudanças na paisagem.
:::
