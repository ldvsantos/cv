...
::: {#fig-restauracao-contexto}
```{mermaid}
%%| fig-width: 9
flowchart LR
    subgraph Restauração Isolada
    A1["Projetos<br>individuais"] --> B1["Fragmentos<br>restaurados<br>dispersos"]
    B1 --> C1["Baixa<br>conectividade<br>mantida"]
    end

    subgraph Restauração Paisagística
    A2["Diagnóstico da<br>paisagem inteira"] --> B2["Priorização<br>espacial<br>(onde restaurar<br>primeiro)"]
    B2 --> C2["Restauração em<br>posições<br>estratégicas"]
    C2 --> D2["Conectividade<br>e funcionalidade<br>recuperadas"]
    end

    style C1 fill:#B71C1C,color:#fff
    style D2 fill:#2E7D32,color:#fff
```
Comparação entre restauração isolada (projetos dispersos, sem planejamento espacial) e restauração paisagística (priorização estratégica com base na estrutura da paisagem).
:::
