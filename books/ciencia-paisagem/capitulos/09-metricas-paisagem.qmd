...
```{dot}
//| label: fig-niveis-metricas
//| fig-cap: "Três níveis hierárquicos de cálculo de métricas da paisagem, mostrando as saídas típicas de cada nível."
//| fig-width: 6.5
digraph {
    rankdir=TB
    bgcolor="transparent"
    ...
    A [label="Mapa de uso e cobertura", fillcolor="#2E7D32", fontcolor="white", color="#1B5E20", penwidth=1.5]
    B [label="Nível da mancha\nAtributos individuais", fillcolor="#BBDEFB", color="#1565C0"]
    C [label="Nível da classe\nAgregação por tipo de cobertura", fillcolor="#FFE0B2", color="#E65100"]
    D [label="Nível da paisagem\nSíntese global", fillcolor="#E1BEE7", color="#6A1B9A"]
    E [label="Área, perímetro,\nforma, área nuclear", fillcolor="#E3F2FD", color="#1565C0"]
    F [label="Nº manchas, área total,\nárea média, agregação", fillcolor="#FFF3E0", color="#E65100"]
    G [label="Diversidade, equabilidade,\ncontagion, borda total", fillcolor="#F3E5F5", color="#6A1B9A"]
...
**Exemplo de cálculo:** Defina-se um raio de busca ecologicamente calibrado para o voo de dispersão de primatas tropicais (ex. 500 m). Em torno do fragmento focal $F_0$, detectam-se dois outros remanescentes dentro deste trânsito: $F_1$ possuindo extensos 100 ha (1.000.000 m²) e tangenciando uma distância curta $h_1 = 100$ m; e $F_2$ com parcos 2 ha (20.000 m²), mais distanciado a $h_2 = 400$ m. O fluxo de vizinhança relativo a $F_1$ equivale a $1.000.000 / (100^2) = 1.000.000 / 10.000 = 100$. O fluxo suprido por $F_2$ é minimizado para $20.000 / (400^2) = 20.000 / 160.000 = 0.125$. O consolidado resulta em $PROX = 100.125$.

Note a assimetria brutal: por ser mais massivo e próximo, $F_1$ fornece esmagadora robustez ao sistema, mitigando a chance e o custo energético da travessia interespecífica da fauna entre manchas. Valores baixos se traduziriam em cenários onde $F_0$ estaria isolado ou ladeado apenas por fragmentos irrisórios em perímetros limítrofes operacionais, desestabilizando fluxos genéticos locais. O raio restritivo deve ser condicionado especificamente pelas taxas de vagilidade faunística ou limites hidrocinéticos, sublinhando que métricas numéricas flutuantes provêm de vetores biológicos reais e auditáveis.
