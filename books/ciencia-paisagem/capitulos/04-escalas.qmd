...
%%| fig-width: 7
flowchart TD
    A["Escala de Observação"] --> B["Grão (resolução)"]
    A --> C["Extensão (abrangência)"]
    B --> D["Espacial: pixel, parcela, fragmento"]
    B --> E["Temporal: dia, mês, ano, década"]
    C --> F["Espacial: bacia, município, bioma"]
    C --> G["Temporal: período de análise"]
```
Componentes da escala de observação na análise da paisagem, mostrando que tanto o grão quanto a extensão possuem dimensão espacial e temporal.
:::
...
%%| fig-width: 7
flowchart TB
    A["Região Biogeográfica<br>(10⁶ ha, 10³ anos)"] --> B["Paisagem<br>(10³–10⁵ ha, 10¹–10² anos)"]
    B --> C["Ecossistema<br>(10¹–10³ ha, 10⁰–10¹ anos)"]
    C --> D["Comunidade/População<br>(10⁻¹–10¹ ha, 10⁻¹–10⁰ anos)"]
...
```
Hierarquia de níveis de organização espacial e suas escalas características de extensão (ha) e tempo (anos). As setas indicam que cada nível superior impõe restrições sobre o inferior, enquanto o inferior fornece os mecanismos.
:::
