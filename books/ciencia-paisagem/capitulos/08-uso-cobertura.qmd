...
::: {#fig-fluxo-classificacao}
```{mermaid}
%%| fig-width: 9
flowchart LR
    A["Imagem pré-processada"] --> B["Definição de classes"]
    B --> C["Coleta de amostras de treinamento"]
    C --> D["Treinamento do classificador"]
    D --> E["Classificação da imagem"]
    E --> F["Pós-processamento<br>(filtro de moda, remoção de ruído)"]
    F --> G["Validação<br>(matriz de confusão)"]
    G -->|Acurácia insuficiente| C
    G -->|Acurácia satisfatória| H["Mapa temático final"]
```
Fluxo de trabalho da classificação supervisionada de uso e cobertura do solo, destacando o laço iterativo de refinamento.
:::
