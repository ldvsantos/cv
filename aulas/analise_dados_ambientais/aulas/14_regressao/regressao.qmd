...
```{mermaid}
%%| fig-width: 10
%%{init: {"theme": "base", "themeVariables": {"fontFamily": "Arial, sans-serif", "primaryTextColor": "#111827", "lineColor": "#111827", "edgeLabelBackground": "#FFFFFF"}}}%%
flowchart TD
    A["Tipo de<br/>variável dependente?"]:::root --> B{"VD"}:::node
  B --> B1["Contínua"]:::option
  B --> B2["Dicotômica<br/>(0/1)"]:::option
  B --> B3["Politômica<br/>nominal"]:::option
  B --> B4["Contagem"]:::option

  B1 --> C{"Quantas VIs?"}:::node
  B2 --> D["Regressão<br/>Logística Binária"]:::result
  B3 --> E["Regressão<br/>Multinomial"]:::result
  B4 --> F["Regressão<br/>de Poisson"]:::leaf

  C --> C1["1 VI"]:::option
  C --> C2["≥ 2 VIs"]:::option
  C1 --> G["Regressão Linear<br/>Simples"]:::leaf
  C2 --> H["Regressão Linear<br/>Múltipla"]:::result

    classDef root fill:#111827,color:#FFFFFF,font-weight:bold,stroke:#000000,stroke-width:2px
    classDef node fill:#FFFFFF,color:#111827,font-weight:bold,stroke:#111827,stroke-width:2px
  classDef option fill:#FEF3C7,color:#111827,font-weight:bold,stroke:#92400E,stroke-width:2px
    classDef leaf fill:#E0F2FE,color:#0F172A,font-weight:bold,stroke:#0369A1,stroke-width:2px
    classDef result fill:#1D4ED8,color:#FFFFFF,font-weight:bold,stroke:#111827,stroke-width:2px
...
![Curva logística — probabilidade × preditor](fig_03_curva_logistica.png){width="100%" .fig-compact}
...
flowchart LR
    ...
    A --> C["Dicotômica"]:::node
    A --> D["Politômica<br/>nominal"]:::node
    A --> E["Contagem"]:::node

  class B,C,D,E option

    B --> F{"Quantas VIs?"}:::node
    F --> |"1"| G["Regressão Linear<br/>Simples"]:::leaf
    F --> |"≥ 2"| H["Regressão Linear<br/>Múltipla"]:::result

    C --> I["Regressão<br/>Logística Binária"]:::result
    D --> J["Regressão<br/>Multinomial"]:::result
    E --> K["Regressão<br/>de Poisson"]:::leaf
