![Uma imagem contendo bolo, comida, diferente, azul&#x0A;&#x0A;O conteúdo gerado por IA pode estar incorreto.](Aspose.Words.0dc411d6-7e35-42bf-a1dd-52dbd0a72c79.001.png)**UNIVERSIDADE ESTADUAL DE FEIRA DE SANTANA**
...
**METODOLOGIA:** O período letivo 2026.1 da Universidade Estadual de Feira de Santana (UEFS) será ofertado na modalidade presencial. A disciplina (04 créditos) será desenvolvida ao longo de 30 encontros, organizados em aulas com 04 horas diárias, compreendendo 02 horas/aula teóricas e 02 horas/aula práticas, conforme o plano de ensino aprovado e o horário institucionalmente estabelecido. 
...
1. **Apresentação da disciplina e fundamentos do conceito de paisagem (4 h)**
   1. Organização do componente, objetivos, critérios e produtos esperados.
   1. Paisagem na Geografia e interfaces: conceito, evolução e usos analíticos.
   1. Distinções operacionais: espaço, território, ambiente e paisagem; paisagem como síntese.
1. **Abordagens integradoras e leitura sistêmica (4 h)**
   1. Geossistema e integração físico-biótica e socioespacial.
   1. Ecologia da paisagem: matriz–mancha–corredor; mosaicos; conectividade.
   1. Pressões antrópicas e transformação da paisagem: implicações analíticas.
1. **Escala, padrão e processo na dinâmica da paisagem (4 h)**
   1. Escalas espaço-temporais, hierarquias e níveis de organização.
   1. Fluxos, conectividade, permeabilidade, bordas e fragmentação.
   1. Resiliência, vulnerabilidade e limiares em paisagens sob pressão.
1. **Cartografia temática e leitura integrada da paisagem (4 h)**
   1. Leitura cartográfica: relevo, drenagem, uso/cobertura, redes, acessibilidade.
   1. Interpretação espacial: padrões, unidades e relações entre elementos.
   1. Exercícios com bases públicas: delimitação preliminar e caracterização.
1. **Interpretação visual e evidências para análise da paisagem (4 h)**
   1. Elementos de interpretação: forma, textura, tonalidade, contexto e associação.
   1. Construção de “matriz de evidências” e roteiro de interpretação.
   1. Atividade prática: síntese descritivo-interpretativa com suporte cartográfico.
1. **Sensoriamento remoto aplicado I: fundamentos operacionais (4 h)**
   1. Imagens orbitais, resoluções e composições; noções de índices e realce.
   1. Uso/cobertura da terra: leitura e classificação (introdução).
   1. Exercício aplicado: identificação de feições e padrões antrópicos/naturais.
1. **Sensoriamento remoto aplicado II: mudanças e trajetórias (4 h)**
   1. Comparação temporal e detecção de mudanças (noções e limitações).
   1. Trajetórias de transformação e interpretação socioambiental.
   1. Produto parcial: mapa/registro interpretativo de mudanças na área de estudo.
1. **Métricas e indicadores espaciais: estrutura e configuração (4 h)**
   1. Heterogeneidade, fragmentação, conectividade, efeito de borda (conceitos).
   1. Indicadores básicos e leitura crítica dos resultados.
   1. Exercício prático com cenário aplicado ao diagnóstico ambiental.
1. **Serious Game I + Avaliação Teórica (4 h)**
   1. *Serious game* (desafio prático orientado): diagnóstico territorial e decisão.
   1. Revisão dirigida dos conteúdos.
   1. **1ª Avaliação (prova teórica individual)**, conforme plano.
1. **Unidades de paisagem e diagnóstico integrado (4 h)**
...
**Quadro 1 – Conteúdo programático por aula de Introdução a Estatística, UFS - 2025.1.**
