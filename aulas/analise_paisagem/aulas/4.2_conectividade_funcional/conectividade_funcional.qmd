...
subtitle: "Métricas, Corredores Ecológicos e Graphab<br>Análise da Paisagem"
author: "Luiz Diego Vidal Santos"
format:
  revealjs:
    logo: ../../../assets/logo-uefs.webp
    footer: "UEFS | Análise da Paisagem | Conectividade Funcional"
    slide-number: true
...
- **Atributo** = área da mancha (aᵢ) como proxy da qualidade do habitat
:::
...
Duas manchas são consideradas **conectadas** se a distância entre elas for ≤ a **capacidade de dispersão** da espécie-alvo (d_max).

| Organismo | d_max típico |
|-----------|:---:|
...
IIC = Σᵢ Σⱼ [aᵢ × aⱼ / (1 + nl_ij)] / A_L²

Onde:

- aᵢ, aⱼ = área das manchas i e j
- nl_ij = número de ligações (arestas) no caminho mais curto entre i e j
...
PC = Σᵢ Σⱼ (aᵢ × aⱼ × p*_ij) / A_L²

Onde p*_ij é o **produto máximo de probabilidades** de dispersão ao longo do caminho ótimo entre i e j.
...
- d_ij = distância de custo entre manchas
- k = constante de dispersão (k = −ln(p_median) / d_median)
:::
...
dPC_k = (PC − PC_sem_mancha_k) / PC × 100
...
4. Capacidade de dispersão da espécie (d_max)
:::
:::
:::
...
2. **Graph → Calculate Metrics**
   ...
   - Distância mediana: d_median = capacidade de dispersão da espécie
...
## Genética da paisagem (*Landscape Genetics*) {.smaller-text}

::: {.columns}
::: {.column width="50%"}
### Da conectividade estrutural à funcional

Métricas como PC e IIC avaliam a conectividade **potencial** (baseada na geometria do mapa). Mas a conectividade é **real**?

A **genética da paisagem** (Manel et al., 2003) integra:

- **Ecologia da paisagem** → métricas espaciais, resistência da matriz
- **Genética de populações** → marcadores moleculares (microssatélites, SNPs)

### Princípio operacional

1. Medir **diferenciação genética** (F_ST) entre populações em manchas distintas
2. Calcular **distância paisagística** (custo, resistência de circuito)
3. Testar se o modelo com heterogeneidade da matriz explica mais variação genética que a simples distância geográfica

> Se distância de custo prediz F_ST melhor que distância euclidiana → **a matriz importa para o fluxo gênico**
:::
...
Estudos de genética da paisagem demonstraram que:

- **Corredores ripários** funcionam como condutos efetivos de fluxo gênico para peixes e anfíbios
- **Conversão floresta → pastagem** aumenta significativamente a diferenciação genética entre populações de primatas
- **Matas de galeria do Cerrado** operam como últimos corredores funcionais para espécies florestais

### Implicação para o Graphab

Os custos de deslocamento atribuídos no Graphab podem ser **calibrados** com dados genéticos → validação empírica dos modelos de resistência.

**Referências:**  
Manel et al. (2003) *Trends Ecol. Evol.*  
Storfer et al. (2007) *Heredity*
:::
:::
:::
...
4. Gere o grafo com d_max = 1500 m (ave florestal)
