...
subtitle: "Sensoriamento Remoto Aplicado II: Mudanças e Trajetórias<br>Curso de Geografia"
author: "Luiz Diego Vidal Santos"
format:
  revealjs:
    logo: ../../../assets/logo-uefs.webp
    footer: "UEFS | Análise da Paisagem | Sensoriamento Remoto II"
    slide-number: true
...
> *"Compreender a paisagem exige leitura do presente E do passado. Padrões atuais são resultado de trajetórias históricas de transformação."* (adaptado de Turner & Gardner, 2015)

A detecção de mudanças complementa a análise estática (Aula 16) com a **dimensão temporal**.
:::
:::
:::
...
## Regimes de distúrbio e dinâmica da paisagem {.smaller-text}

::: {.columns}
::: {.column width="50%"}
### Distúrbios como forçantes de heterogeneidade

Além das forças motrizes socioeconômicas, a paisagem é modulada por **regimes de distúrbio** (Turner, 2010), definidos pelo padrão temporal e espacial de eventos que removem biomassa ou redefinem condições abióticas.

Um regime de distúrbio é caracterizado por:

- **Frequência**  -  intervalo médio entre eventos
- **Intensidade**  -  magnitude do impacto
- **Extensão**  -  área afetada por evento
- **Sazonalidade**  -  época preferencial de ocorrência

### Categorias relevantes no Brasil

| Distúrbio | Exemplo | Efeito na paisagem |
|-----------|---------|--------------------|
| **Fogo** | Queimadas no Cerrado/Amazônia | Seleção de espécies rebrotantes, conversão de fisionomias |
| **Eventos climáticos extremos** | Seca 2015-16 na Amazônia | Mortalidade seletiva de árvores, deslizamentos |
| **Perturbações antrópicas agudas** | Barragens, mineração, rodovias | Fragmentação abrupta, redefinição de drenagem |
...
A interação entre regime de distúrbio e extensão da paisagem determina o comportamento do sistema:

- **Equilíbrio:** distúrbios frequentes mas pequenos em relação à paisagem → proporção de cada estágio sucessional permanece estável
- **Não-equilíbrio:** distúrbios raros mas extensos → alteram simultaneamente grande parte da paisagem (ex.: incêndios catastróficos)

### Implicação para detecção de mudanças

A detecção de mudanças por sensoriamento remoto deve distinguir:

1. Mudanças **dirigidas por distúrbio natural** (fogo, enchente) → recuperação potencial
2. Mudanças **dirigidas por conversão antrópica** (desmatamento) → tendência irreversível sem intervenção
3. Mudanças **interativas** (fogo em borda de fragmento desmatado) → retroalimentação positiva

> *"Distúrbios não são desvios do normal  -  são parte integral da dinâmica da paisagem."*  -  Turner (2010, adaptado)
:::
:::
:::
...
### Exemplo interpretativo ⚠️ *cenário didático hipotético*
...
## Para a próxima aula (Aula 18) {.smaller-text}

::: {.columns}
::: {.column width="50%"}
### Atividade: produto parcial

Na Aula 18, você produzirá um **registro interpretativo de mudanças** na área de estudo:
...
## Síntese da Aula 17 {.smaller-text}
