...
subtitle: "Fluxos, Fragmentação, Resiliência e Vulnerabilidade<br>Curso de Geografia"
author: "Luiz Diego Vidal Santos"
format:
  revealjs:
    logo: ../../../assets/logo-uefs.webp
    footer: "UEFS | Análise da Paisagem | Fluxos, Fragmentação e Resiliência"
    slide-number: true
...
## Débito de extinção: a conta atrasada {.smaller-text .scrollable}

::: {.columns}
::: {.column width="50%"}
### O conceito

A resposta das espécies à perda de hábitat **não é imediata** — existe um *time lag* (atraso) entre a perturbação e a extinção local efetiva.

> Analogia: é como o **cartão de crédito** — você gastou agora, mas a fatura chega depois. A natureza foi impactada hoje; as extinções só serão observadas anos ou décadas à frente.

### Magnitude do *time lag* (revisão de 83 estudos)

- Variação: **5 a 570 anos**
- Mediana em torno de **≈ 100 anos** para a maioria dos grupos
- Atinge entre **9 % e 90 %** das espécies atuais, dependendo do grupo
- **Maior** em organismos longevos (onças, árvores) e fragmentos pequenos
- **Menor** em organismos de ciclo curto (insetos, anuárias)
:::

::: {.column width="50%"}
::: {.info-box}
**Hipótese da quantidade de hábitat (Fahrig, 2003 ss.)**

A quantidade total de hábitat no entorno explica mais a riqueza de espécies do que a configuração (número/forma dos fragmentos).

→ Implicação: **proteger área é prioridade**, ainda que a configuração também importe.
:::

::: {.highlight-box}
**Caso brasileiro: Pontal do Paranapanema (SP)**

A composição atual da comunidade de **aves** respondeu mais à estrutura da paisagem de **1978** do que à estrutura atual:

- *time lag* observado: **≈ 25 anos**
- Pequenos fragmentos: extinções locais futuras já "contratadas"
- Persistência tende a se concentrar nos **grandes fragmentos** (ex.: Morro do Diabo)
:::

**Implicação para a gestão:** estamos manejando hoje paisagens cuja resposta biológica plena só será visível décadas à frente.
:::
:::
...
> Na **Aula 19** trabalharemos com essas métricas em exercício prático.
:::
:::
:::
...
## Modelos neutros e validação de padrões {.smaller-text}

::: {.columns}
::: {.column width="50%"}
### O problema

Quando calculamos métricas de fragmentação, como saber se o padrão observado é resultado de um **processo ecológico real** ou apenas de uma **distribuição aleatória** de cobertura?

### Modelos neutros da paisagem

Gardner et al. (1987) propuseram gerar **paisagens sintéticas aleatórias** com a mesma proporção de habitat (PLAND) e extensão da paisagem real.

**Procedimento:**

1. Gerar 100-1000 paisagens aleatórias com mesmo PLAND
2. Calcular métricas para cada paisagem simulada
3. Construir distribuição nula
4. Comparar valores observados com a distribuição nula

> Se NP_real >> NP_neutro → processo de fragmentação ativo  
> Se AI_real >> AI_neutro → autocorrelação espacial (sucessão, legislação)
:::

::: {.column width="50%"}
::: {.info-box}
**Limiar de percolação teórico
**

Em paisagens binárias aleatórias (With & King, 1997), o **limiar de percolação** ocorre em PLAND ≈ 59,28%.

Abaixo desse valor, a conectividade colapsa em paisagens aleatórias. Se a paisagem real mantém conectividade com PLAND < 59%, isso indica que a **configuração espacial** (não apenas a quantidade) está preservando a conectividade.

### Aplicação prática

Em paisagens de Mata Atlântica com PLAND entre 5-25%, a agregação de floresta em topos de morro e ao longo de cursos d'água é **significativamente superior** à esperada por distribuição aleatória → evidência do efeito combinado de:

- Topografia
- Legislação de APPs
- Histórico de uso (preservação de áreas íngremes)

**Referências:**  
Gardner et al. (1987) *Landscape Ecology*  
With & King (1997) *Oikos*
:::
:::
:::
...
- **Aula 10**  -  Cartografia temática aplicada
- **Aula 11**  -  Exercício: delimitação e caracterização da área
- **Aula 12**  -  Interpretação visual da paisagem
- **Aula 13**  -  Matriz de evidências e síntese descritivo-interpretativa

**Avaliação (Aula 14)  -  15/04**

Questões objetivas e discursivas sobre os conteúdos das **Aulas 03 a 13**.
